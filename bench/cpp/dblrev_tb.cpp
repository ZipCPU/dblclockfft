...
int	main(int argc, char **argv, char **envp) {
	...
	for(int k=0; k<4*(FFTSIZE); k++) {
		...
		if ((syncd)&&((dblrev->o_out_0&FFTMASK) != bitrev(FFTBITS, 2*(k-BREV_OFFSET)))) {
			fprintf(stdout, "FAIL: BITREV.0 of k (%2x) = %2lx, not %2lx\n",
				k, dblrev->o_out_0, bitrev(FFTBITS, 2*(k-BREV_OFFSET)));
			exit(-1);
		}

		if ((syncd)&&((dblrev->o_out_1&FFTMASK) != bitrev(FFTBITS, 2*(k-BREV_OFFSET)+1))) {
			fprintf(stdout, "FAIL: BITREV.1 of k (%2x) = %2lx, not %2lx\n",
				k, dblrev->o_out_1, bitrev(FFTBITS, 2*(k-BREV_OFFSET)+1));
			exit(-1);
		}
	}
